...
const DayOfWeek = ( { Day, date, imageUrl, imageAlt, Range, className } ) => (
    <div className={ `flex flex-col items-center justify-around max-xs:flex-row ${ className }` }>
        <Name value={ Day } className="font-extrabold text-2xl max-xs:text-left max-xs:basis-1/2 max-xs:text-sm max-xs:font-semibold" />
