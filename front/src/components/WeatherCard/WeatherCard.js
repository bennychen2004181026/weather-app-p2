...
import useSnackbarHelper from '../../hooks/useSnackbar';

const WeatherCard = () =>
{
    const [ isLoading, setIsLoading ] = useState( false );
    const { weatherDataList } = useContext( WeatherDataContext );
    const { data, isLoading: dataIsLoading, error: dataError } = weatherDataList;
    ...
    {
        return (
            <div role="status" className="max-h-[900px] max-md:max-h-[1500px] max-xl:w-[90%] max-lg:w-[95%] max-md:h-full max-md:flex max-md:flex-col max-w-[1650px] p-6 max-xs:p-2 rounded-3xl w-4/5 h-4/5 mx-auto bg-slate-100 shadow-blue-800 overflow-hidden flex justify-center items-center">
